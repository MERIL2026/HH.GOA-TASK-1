...
import cardGLBUrl from '../../assets/lanyard/card.glb';
import lanyardPngUrl from '../../assets/lanyard/lanyard.png';
import './Lanyard.css';

extend({ MeshLineGeometry, MeshLineMaterial });

const CARD_GLB: string = cardGLBUrl;
const LANYARD_PNG: string = lanyardPngUrl;
...
function Band({
  ...
  const cardMap = useMemo(() => {
    ...
    const composite = new THREE.CanvasTexture(offscreen);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      [j1, j2].forEach((ref: React.MutableRefObject<any>) => {
